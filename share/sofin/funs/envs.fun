...
compiler_setup () {
    ...
    case "${SYSTEM_NAME}" in
        FreeBSD)
            DEFAULT_COMPILER_FLAGS="${HARDEN_CFLAGS} ${PIC_CFLAGS} ${PIE_CFLAGS} ${HARDEN_CFLAGS_PRODUCTION} ${CMACROS}"
            DEFAULT_LINKER_FLAGS="${HARDEN_LDFLAGS_PRODUCTION} ${PIC_LDFLAGS} ${PIE_LDFLAGS}"
            ;;
