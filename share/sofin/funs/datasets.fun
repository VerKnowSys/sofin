    ...
    if [ -f "${FILE_CACHE_DIR}${_psfin_snapfile}" ]; then
        ...
        if [ "${?}" = "0" ]; then
            ...
            if [ "${?}" = "0" ]; then
                ...
                retry "${SSH_BIN} ${DEFAULT_SSH_OPTS} -p ${MAIN_SSH_PORT} ${SOFIN_NAME}@${_psmirror} \"cd ${COMMON_BINARY_REMOTE} && ${MV_BIN} ${_psfin_snapfile}${DEFAULT_PARTIAL_FILE_EXT} ${_psfin_snapfile}\"" && \
